...
const VoiceChat: React.FC<VoiceChatProps> = ({ recordingId }) => {
    ...
    const connect = async () => {
        try {
            ...
            // In development: use local proxy or localhost
            // In production: use Render backend directly for WebSocket compatibility
            const isDev = window.location.hostname === 'localhost';
            const backendUrl = isDev
                ? 'http://localhost:10000'
                : 'https://kipu-backend-8006.onrender.com';

            // 1. Initialize session (HTTP POST)
            const apiUrl = `${backendUrl}/api/voice/init/${recordingId}`;
            console.log('[Voice] Connecting to API:', apiUrl);
            ...
            if (!response.ok) {
                ...
                throw new Error(`Error ${response.status}: ${errorText || 'Failed to initialize'}`);
            }
            ...
            const wsUrl = `${wsProtocol}//${wsHost}/voice?sessionId=${sessionId}`;
